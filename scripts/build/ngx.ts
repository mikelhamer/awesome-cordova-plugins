...
export function transpileNgxCore() {
  getProgram([path.resolve(ROOT, 'src/@awesome-cordova/core/index.ts')]).emit({
...
export function generateLegacyBundles() {
  [
    path.resolve(ROOT, 'dist/@awesome-cordova/core/index.js'),
    ...PLUGIN_PATHS.map(p =>
    ...
    rollup
      .rollup({
        ...
        external: ['@angular/core', '@awesome-cordova/core', 'rxjs', 'tslib'],
      })
      .then(bundle =>
        bundle.write({
...
// remove reference to @awesome-cordova/core decorators
...
function removeIonicNativeDecorators(node: any) {
  if (node.decorators && node.decorators.length) {
    node.decorators = node.decorators.filter(
      (d: { expression: { module: string } }) => d.expression.module !== '@awesome-cordova/core'
    );
  }
