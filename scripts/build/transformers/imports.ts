...
function transformImports(file: ts.SourceFile, ctx: ts.TransformationContext, ngcBuild?: boolean) {
  ...
  // find the @awesome-cordova/core import statement
  const importStatement = (file.statements as any).find((s: any) => {
    return s.kind === ts.SyntaxKind.ImportDeclaration && s.moduleSpecifier.text === '@awesome-cordova/core';
  });

  // we're only interested in files containing @awesome-cordova/core import statement
